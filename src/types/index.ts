...
export interface Session { id: string; projectId: string; title: string; updatedAt: number; status: CodexStatus; }
export interface Settings { theme: 'light' | 'dark' | 'system'; language?: 'zh-CN' | 'en-US'; adapter: 'mock' | 'cli'; model: string; permission: 'read-only' | 'workspace-write' | 'full-access'; notifications: boolean; autoScroll: boolean; compact: boolean; logExpanded: boolean; diffMode: 'inline' | 'side-by-side'; fontSize: number; cliPath: string; userAvatar?: string; assistantAvatar?: string; }
