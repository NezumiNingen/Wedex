...
export interface CodexAdapter { check(cliPath: string): Promise<{ installed: boolean; version?: string; error?: string }>; run(cwd: string, prompt: string, cliPath: string, language: Language | undefined, attachmentPaths: string[], onEvent: (event: CodexEvent) => void): Promise<void>; cancel(): Promise<void>; }
...
export const cliAdapter: CodexAdapter = {
  ...
  async run(cwd, prompt, cliPath, language, attachmentPaths, onEvent) { const unlisten = await listen<WireEvent>('codex-event', ({ payload }) => { const event = normalise(payload); onEvent(event); if (event.type === 'completed' || event.type === 'error') void unlisten(); }); onEvent({ type: 'status', content: t(language, 'startingCli'), status: 'starting' }); try { await invoke('start_codex_task', { cwd, prompt, sandbox: 'workspace-write', cliPath, attachmentPaths }); } catch (error) { void unlisten(); throw error; } },
